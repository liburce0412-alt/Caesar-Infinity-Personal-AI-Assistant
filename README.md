...
<h1 align="center">Caesar∞ · 懂你日常的私人 AI 助手</h1>

<p align="center"><strong>把手环数据和日常记录，变成能聊、能追问的个人反馈。</strong></p>

<p align="center">
  Android · 小米健康接入 · 本地对话留存 · 自选本地或云端模型
</p>

<p align="center">
  <a href="#先看使用场景">看看怎么用</a> ·
  <a href="#参与内测">下载与内测</a> ·
  <a href="#快速开始">开发者构建</a> ·
...
2. **再问一句。** 配置好云端模型后，在 AI 对话的上下文设置中勾选「本次附带今日健康摘要（发送后自动关闭）」，尝试问：“根据今天已有的睡眠和活动数据，帮我整理一下今日状态，并指出缺了哪些信息。”
3. **按需开启自动反馈。** 在「我的 → 健康自动化」选择可用的 DeepSeek 或 Gemini 模型、检查间隔，并授权必要的今日汇总。App 在前台检查到数据变化时生成短消息。

以上是体验步骤与示例提问，具体反馈取决于已同步数据与所选模型。**健康自动化当前仅在 App 前台运行，不提供后台持续监测或医疗诊断。** 没有手环也可以体验对话、时间记录和个人记忆。

### 为什么用 Caesar∞

| 你想做的事 | Caesar∞ 提供什么 | 使用条件 |
| --- | --- | --- |
| 围绕自己的健康数据继续追问 | 在授权后把必要的今日摘要带入对话，首页显示来源与新鲜度 | 先同步数据；云端对话需明确勾选附带摘要 |
| 用自然语言整理日常记录 | Agent 可读取、新建和编辑 App 内的时间记录 | 依赖所选模型的工具调用能力，操作受参数和确认策略约束 |
| 决定 AI 记住什么 | 长期记忆先提议、由你确认，并可查看、修改和删除 | 不自动把原始健康序列写入记忆 |
| 自己选择推理方式 | 下载本地模型，或配置个人云端模型 Key | 本地模型另行下载；云端调用需要网络与可用服务 |

对话记录与导入图片保存在本机。**选择云端模型时，完成当轮回答所需的消息仍会发送给所选服务；本地留存不等于所有功能完全离线。** 详细说明见[模型与联网](#模型与联网)与[隐私与安全](#隐私与安全)。

## 参与内测

### 安装前准备

- **设备：** 当前重点适配 Xiaomi 15 Pro（Android 16、16 GB RAM），其他设备的兼容性与本地模型表现仍需验证。
- **模型：** 本地模式需要另行下载模型并预留存储空间；云端模式需要自己的可用 API Key，费用与网络可达性取决于所选服务。健康自动化需要 DeepSeek 或 Gemini。
- **健康接入：** 小米手环体验需要 Mi Fitness 已完成同步，并在 App 内配置小米账户凭据；完整步骤见[手环接入](#小米手环与健康数据)。
...
体验版可能使用独立的预发布条目与 debug 签名，请以对应发布说明为准。模型权重、个人密钥、健康原始序列、设备日志和 APK 均不会进入源码历史。
